...
export default function HistorialView({ syncRuns }: { syncRuns: SyncRun[] }) {
  ...
  const [expandedId, setExpandedId] = useState<string | null>(syncRuns[0]?.id ?? null);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass rounded-xl p-4 flex items-center gap-3">
        ...
        />
        <span className="text-xs text-zinc-600">{filteredRuns.length} sync{filteredRuns.length !== 1 ? "s" : ""}</span>
      </div>
      ...
      <div className="space-y-3">
        {filteredRuns.map((run) => {
          ...
          const date = new Date(run.syncedAt);
            ...
            <div key={run.id} className="glass rounded-2xl overflow-hidden">
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-2 rounded-lg ${isOpen ? "bg-indigo-500/10" : "bg-white/[0.03]"}`}>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">{date.toLocaleTimeString("es-AR")} · {run.total} servidores</p>
                  </div>
                </div>
                <div className="flex items-center gap-4 mr-2">
                  <div className="flex items-center gap-1.5">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                    <span className="text-xs text-emerald-400">{run.success}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <XCircle className="w-3.5 h-3.5 text-rose-400" />
                    <span className="text-xs text-rose-400">{run.errors}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <AlertCircle className="w-3.5 h-3.5 text-zinc-500" />
                    <span className="text-xs text-zinc-500">{run.noData}</span>
                  </div>
